...
module.exports = class DbLoggerRuntime {
  ...
  resetRun() {
    ...
    this.sessionId = null;
    this.suiteId = null;
    // timestamp is empty in Nightwatch 1.2.4
  ...
  async processSuite({ className, module, moduleName }) {
    ...
    // Nightwatch 1.2.4 - timestamp property is empty. Using current time as start.
    const timestampAsNumber = utils.parseDate(timestamp);

    if (!isNaN(timestampAsNumber)) {
      this.currentTimestamp = timestampAsNumber;
    }
  ...
  async processCompleted(suiteId, completed) {
    ...
    for (let testCase in completed) {
      ...
      // Hit a bug in Nightwatch 1.2.4, the time of a testcase was negative
      // At least protect against times in the past
      this.currentTimestamp += Math.abs(utils.parseTime(time));
